...
def build_server_config(argv: Sequence[str] | None = None) -> MCPServerConfig:
    env_transport = (os.environ.get("ANAMNESIS_MCP_TRANSPORT") or "stdio").strip().lower()
    if env_transport not in TRANSPORTS:
        raise ValueError(f"ANAMNESIS_MCP_TRANSPORT must be one of: {', '.join(TRANSPORTS)}")

    env_log_level = (os.environ.get("ANAMNESIS_MCP_LOG_LEVEL") or "INFO").strip().upper()
    if env_log_level not in LOG_LEVELS:
        raise ValueError(f"ANAMNESIS_MCP_LOG_LEVEL must be one of: {', '.join(LOG_LEVELS)}")

    parser = argparse.ArgumentParser(description="Run the Anamnesis MCP server")
    parser.add_argument("--transport", choices=TRANSPORTS, default=env_transport)
    parser.add_argument("--host")
    parser.add_argument("--port", type=int)
    parser.add_argument("--mount-path", default=os.environ.get("ANAMNESIS_MCP_MOUNT_PATH") or "/")
    parser.add_argument("--sse-path", default=os.environ.get("ANAMNESIS_MCP_SSE_PATH") or "/sse")
    parser.add_argument("--message-path", default=os.environ.get("ANAMNESIS_MCP_MESSAGE_PATH") or "/messages/")
    parser.add_argument("--streamable-http-path", default=os.environ.get("ANAMNESIS_MCP_STREAMABLE_HTTP_PATH") or "/mcp")
    parser.add_argument("--log-level", choices=LOG_LEVELS, default=env_log_level)
    parser.add_argument("--debug", action=argparse.BooleanOptionalAction, default=_env_flag("ANAMNESIS_MCP_DEBUG", False))
    parser.add_argument("--json-response", action=argparse.BooleanOptionalAction, default=_env_flag("ANAMNESIS_MCP_JSON_RESPONSE", False))
    parser.add_argument("--stateless-http", action=argparse.BooleanOptionalAction, default=_env_flag("ANAMNESIS_MCP_STATELESS_HTTP", False))
    args = parser.parse_args(argv)

    transport = args.transport
    host = args.host or os.environ.get("ANAMNESIS_MCP_HOST") or os.environ.get("HOST") or _default_host(transport)
    port = args.port if args.port is not None else (_env_int("ANAMNESIS_MCP_PORT", "PORT") or 8000)
...
def create_server(config: MCPServerConfig | None = None):
    ...
    def memory_sql(sql: str, db_path: str | None = None, read_only: bool = True) -> str:
        return _json(service.sql(sql, db_path=db_path, read_only=read_only, backend="uqa"))
